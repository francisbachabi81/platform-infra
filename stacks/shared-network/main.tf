...
# App Gateway Stage 2 (per-env toggle): UAMI + KV access + SSL + listeners/rules
locals {
  # Stage2 runs only if AppGW exists AND stage2 is globally enabled
  appgw_stage2_enabled = local.appgw_enabled && var.appgw_enable_stage2

  # Which env keys exist for this plane
  appgw_env_keys = local.is_nonprod ? toset(["dev", "qa"]) : toset(["prod", "uat"])

  # Defaults if caller didn't provide the map
  appgw_stage2_env_enabled_effective = merge(
    { dev = true, qa = true, uat = true, prod = true },
    var.appgw_stage2_env_enabled
  )

   # Base subdomain depends on product:
  # - horizon -> horizon.intterra.io
  # - pub     -> public.intterra.io
  appgw_base_subdomain = var.product == "pub" ? "public" : "horizon"

  # Hostnames by env (overrideable via var if you want later)
  appgw_hosts = merge(
    {
      dev  = "dev.${local.appgw_base_subdomain}.intterra.io"
      qa   = "qa.${local.appgw_base_subdomain}.intterra.io"
      uat  = "uat.${local.appgw_base_subdomain}.intterra.io"
      prod = "${local.appgw_base_subdomain}.intterra.io"
    },
    var.appgw_hostnames_by_env
  )

  # Base site definitions for the active plane
  # - backend IP comes from your existing var.appgw_backend_ips_by_env
  # - cert secret name is NEW (var.appgw_ssl_cert_secret_name_by_env)
  appgw_sites_plane = {
    for env in local.appgw_env_keys :
    env => {
      host             = local.appgw_hosts[env]
      backend_ip       = try(var.appgw_backend_ips_by_env[env], null)
      cert_secret_name = try(var.appgw_ssl_cert_secret_name_by_env[env], null)
      enabled          = try(local.appgw_stage2_env_enabled_effective[env], false)
    }
  }

  # Only configure envs that are enabled AND have required inputs
  appgw_sites_effective = {
    for env, s in local.appgw_sites_plane :
    env => s
    if s.enabled && s.backend_ip != null && trimspace(s.backend_ip) != "" && s.cert_secret_name != null && trimspace(s.cert_secret_name) != ""
  }

  # Stage2 "effective" = enabled + we have at least one env site to configure
  appgw_stage2_effective = local.appgw_stage2_enabled && length(local.appgw_sites_effective) > 0

  # UAMI name (overrideable)
  appgw_stage2_uami_name = coalesce(
    var.appgw_stage2_uami_name,
    "uami-${var.product}-${local.plane_code}-${var.region}-appgw-stage2-01"
  )
}

# Create UAMI for AppGW to read cert secrets from Key Vault
resource "azurerm_user_assigned_identity" "appgw_stage2" {
  count               = local.appgw_stage2_effective ? 1 : 0
  name                = local.appgw_stage2_uami_name
  location            = var.location
  resource_group_name = local.appgw_hub_rg
...
# Grant the UAMI permissions to read secrets/certs from the Key Vault
resource "azurerm_key_vault_access_policy" "appgw_stage2" {
  count        = local.appgw_stage2_effective ? 1 : 0
  key_vault_id = var.appgw_key_vault_id
  tenant_id    = var.hub_tenant_id
  object_id    = azurerm_user_assigned_identity.appgw_stage2[0].principal_id

  # AppGW needs to read the cert material
  secret_permissions      = ["Get", "List"]
  certificate_permissions = ["Get", "List"]

  depends_on = [
    azurerm_user_assigned_identity.appgw_stage2
  ]
}

# Stage2 config: certs + listeners + rules per enabled env
#
# IMPORTANT:
# - This assumes you have (or will add) a module that can "attach" stage2 config
#   to an existing gateway created by module.appgw.
# - If your existing ../../modules/app-gateway already supports stage2 inputs,
#   you can fold these inputs into module.appgw instead and remove this module.
module "appgw_stage2" {
  count  = local.appgw_stage2_effective ? 1 : 0
  source = "../../modules/app-gateway-stage2"

  # Existing gateway (from your module.appgw)
  application_gateway_id   = module.appgw[0].id
  application_gateway_name = module.appgw[0].name
  resource_group_name      = local.appgw_hub_rg
  location                 = var.location

  # Identity + Key Vault
  user_assigned_identity_id = azurerm_user_assigned_identity.appgw_stage2[0].id
  key_vault_id              = var.appgw_key_vault_id

  # Per-env sites to configure
  # Each entry includes: host, backend_ip, cert_secret_name
  sites = local.appgw_sites_effective

  # Optional knobs you may want in the stage2 module
  ssl_policy_name = var.appgw_ssl_policy_name
  ...
  depends_on = [
    module.appgw,
    azurerm_key_vault_access_policy.appgw_stage2
  ]
}
