...
: "${DEV_SUB:?set DEV_SUB}"        # dev subscription
: "${QA_SUB:?set QA_SUB}"          # qa subscription
: "${UAT_SUB:?set UAT_SUB}"        # uat subscription
: "${PROD_SUB:?set PROD_SUB}"      # prod subscription
...
STATE_KEY="shared-network/${PRODUCT_IN}/${PLANE}/terraform.tfstate"
...
# ---------------------------
# Switch to HUB subscription + PLAN
# ---------------------------
echo "🎯 az account set (HUB_SUB=$HUB_SUB)"
...
# env | grep -E '^(PLANE_IN|PRODUCT_IN|TENANT_ID|STATE_SUB|HUB_SUB|DEV_SUB|QA_SUB|UAT_SUB|PROD_SUB)='
