...
# Diagnostics – common (to Log Analytics)
locals {
  _la_id = local.law_workspace_id

  # One place to define what to collect for each resource
  diag_targets = {
    # Key Vault
    kv1 = {
      id      = try(module.kv1[0].id, null)
      name    = "kv1-diag"
      logs    = ["AuditEvent"]
      metrics = ["AllMetrics"]
    }
    ...
    # Storage Account
    sa1 = {
      id      = try(module.sa1[0].id, null)
      name    = "sa1-diag"
      logs    = ["StorageRead", "StorageWrite", "StorageDelete"]
      metrics = ["Transaction"]
    }

    # Cosmos DB (Account-level). Containers/DBs typically inherit — collect at account.
    cosmos1 = {
      id      = try(module.cosmos1[0].id, null)
      name    = "cosmos1-diag"
      # Log categories vary by API; keep metrics-only unless you know exact API categories.
      logs    = []
      metrics = ["AllMetrics"]
    }
    ...
    sbns1 = {
      id      = try(module.sbns1[0].namespace_id, module.sbns1[0].id, null)
      name    = "sbns1-diag"
      # Categories differ across SKUs – metrics-only is safe everywhere
      logs    = []
      metrics = ["AllMetrics"]
    }

    # App Service Plan (serverfarm) – metrics only
    plan1_func = {
      id      = try(module.plan1_func[0].id, null)
      name    = "asp-diag"
      logs    = []
      metrics = ["AllMetrics"]
    }
    ...
    # Function Apps (sites)
    funcapp1 = {
      id      = try(module.funcapp1[0].id, null)
      name    = "funcapp1-diag"
      # Function apps reliably expose this log category; add more if you want (e.g. AppServiceHTTPLogs)
      logs    = ["FunctionAppLogs"]
      metrics = ["AllMetrics"]
    }
    funcapp2 = {
      id      = try(module.funcapp2[0].id, null)
      name    = "funcapp2-diag"
      logs    = ["FunctionAppLogs"]
      metrics = ["AllMetrics"]
    }
    ...
    eventhubns = {
      id      = try(module.eventhub[0].namespace_id, null)
      name    = "eventhubns-diag"
      logs    = []
      metrics = ["AllMetrics"]
    }

    # Cosmos DB for PostgreSQL (Citus) – metrics only (log categories vary)
    cdbpg1 = {
      id      = try(module.cdbpg1[0].id, null)
      name    = "cdbpg1-diag"
      logs    = []
      metrics = ["AllMetrics"]
    }
    ...
    # PostgreSQL Flexible
    pgflex_primary = {
      id      = try(module.postgres[0].id, null)
      name    = "pgflex-primary-diag"
      logs    = ["PostgreSQLLogs"]     # valid for Flexible Server
      metrics = ["AllMetrics"]
    }
    pgflex_replica = {
      id      = try(module.postgres_replica[0].id, null)
      name    = "pgflex-replica-diag"
      logs    = ["PostgreSQLLogs"]
      metrics = ["AllMetrics"]
    }
    ...
    # Redis Cache
    redis1 = {
      id      = try(module.redis1[0].id, null)
      name    = "redis1-diag"
      logs    = []                     # categories differ; metrics are universal
      metrics = ["AllMetrics"]
    }
  }

  # Only keep targets where the resource exists and LAW is known
  diag_targets_filtered = {
    for k, v in local.diag_targets :
    k => v
    if v.id != null && local._la_id != null
  }
}

resource "azurerm_monitor_diagnostic_setting" "common" {
  for_each                   = local.diag_targets_filtered
  name                       = each.value.name
  target_resource_id         = each.value.id
  log_analytics_workspace_id = local._la_id

  # Logs (if any were specified)
  dynamic "enabled_log" {
