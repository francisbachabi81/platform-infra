...
module "sbns1" {
  ...
  privatelink_subnet_id = local.sb_is_premium ? try(local.subnet_ids["privatelink"], null) : null
  private_dns_zone_id   = local.sb_is_premium ? try(local.zone_ids["privatelink.servicebus.windows.net"], null) : null
...
module "cdbpg1" {
  ...
  enable_private_endpoint = var.cdbpg_enable_private_endpoint
  privatelink_subnet_id   = try(local.subnet_ids["privatelink-cdbpg"], null)
  private_dns_zone_id     = try(local.zone_ids["privatelink.postgres.cosmos.azure.com"], null)
...
module "postgres_replica" {
  ...
  delegated_subnet_id = try(local.subnet_ids[var.pg_delegated_subnet_name], null)
  private_dns_zone_id = try(local.zone_ids["privatelink.postgres.database.azure.com"], null)

  replica_enabled  = true
  source_server_id = module.cdbpg1[0].id

  tags = merge(local.tags_common, local.tags_postgres, var.tags, { role = "replica" })
  depends_on = [module.cdbpg1]
}
...
module "funcapp1" {
  ...
  enable_scm_private_endpoint = true
...
module "eventhub" {
  ...
  private_dns_zone_id           = local.zone_ids["privatelink.servicebus.windows.net"]
